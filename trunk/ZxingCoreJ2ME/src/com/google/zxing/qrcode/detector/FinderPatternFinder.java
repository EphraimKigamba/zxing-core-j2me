...
/**
 * <p>This class attempts to find finder patterns in a QR Code. Finder patterns
 * are the square markers at three corners of a QR Code.</p>
 *
 * <p>This class is thread-safe but not reentrant. Each thread must allocate its
 * own object.
 *
 * @author Sean Owen
 */
public class FinderPatternFinder {
    ...
    /**
     * <p>Creates a finder that will search the image for three finder
     * patterns.</p>
     *
    ...
    protected final Vector getPossibleCenters() {
        return possibleCenters;
    }

    //final FinderPatternInfo find(Map<DecodeHintType,?> hints) throws NotFoundException {
    final FinderPatternInfo find(Hashtable hints) throws NotFoundException {
        ...
        Vector patternInfo = selectBestPatterns();
    ...
    /**
     * <p>Like {@link #crossCheckVertical(int, int, int, int)}, and in fact is
     ...
     * {@link #CENTER_QUORUM} times, and whose module size differs from the
     * average among those patterns the least
     * @throws NotFoundException if 3 such finder patterns do not exist
     */
    //private FinderPattern[] selectBestPatterns() throws NotFoundException {
    private Vector selectBestPatterns() throws NotFoundException {
        ...
        if (startSize > 3) {
            ...
            SortFurthestFromAverageComparator(possibleCenters, average);
        ...
        if (possibleCenters.size() > 3) {
            ...
            float totalModuleSize = 0.0f;
            for (int i = 0; i < possibleCenters.size(); i++) {
                FinderPattern possibleCenter = (FinderPattern) possibleCenters.elementAt(i);
            ...
            SortCenterComparator(possibleCenters, average);
        ...
        result.addElement(possibleCenters.elementAt(0));
        result.addElement(possibleCenters.elementAt(1));
        result.addElement(possibleCenters.elementAt(2));
        return result;
...
////                };
    ...
    private void SortCenterComparator(Vector src, float average) {
           for (int i = 0; i < src.size(); i++) {
            for (int j = i + 1; j < src.size(); j++) {
                FinderPattern center1 = (FinderPattern) src.elementAt(i);
                FinderPattern center2 = (FinderPattern) src.elementAt(j);
                int comp = CenterComparatorCompare(center1, center2, average);
                if (comp == 0) {
                } else if (comp > 0) {//>
                    src.setElementAt(center2, i);
                    src.setElementAt(center1, j);
                } else if (comp == -1) {//<
                }
            }
        }
...
//        for (int i = 0; i < src.size(); i++) {
//            FinderPattern center1;
//            FinderPattern center2;
//            if (i + 1 < src.size()) {
//                center1 = (FinderPattern) src.elementAt(i);
//                center2 = (FinderPattern) src.elementAt(i + 1);
//
//                int comp = CenterComparatorCompare(center1, center2, average);
//                if (comp == 0) {
//                } else if (comp == 1) {//>
//                    src.setElementAt(center2, i);
//                    src.setElementAt(center1, i + 1);
//                } else if (comp == -1) {//<
//                }
//
//            } else {
//                center1 = (FinderPattern) src.elementAt(src.size() - 1);
//                center2 = (FinderPattern) src.elementAt(0);
//
//                int comp = CenterComparatorCompare(center1, center2, average);
//                if (comp == 0) {
//                } else if (comp == 1) {//>
//                    src.setElementAt(center2, src.size() - 1);
//                    src.setElementAt(center1, 0);
//                } else if (comp == -1) {//<
//                }
//            }
//
...
//        }
    ...
    private void SortFurthestFromAverageComparator(Vector src, float average) {
        for (int i = 0; i < src.size(); i++) {
            for (int j = i + 1; j < src.size(); j++) {
                FinderPattern center1 = (FinderPattern) src.elementAt(i);
                FinderPattern center2 = (FinderPattern) src.elementAt(j);
                int comp = FurthestFromAverageComparatorCompare(center1, center2, average);
                if (comp == 0) {
                } else if (comp > 0) {//>
                    src.setElementAt(center2, i);
                    src.setElementAt(center1, j);
    ...
    private int FurthestFromAverageComparatorCompare(FinderPattern center1, FinderPattern center2, float average) {
        float dA = Math.abs(center2.getEstimatedModuleSize() - average);
        float dB = Math.abs(center1.getEstimatedModuleSize() - average);
        return dA < dB ? -1 : dA == dB ? 0 : 1;
    }

    private int CenterComparatorCompare(FinderPattern center1, FinderPattern center2, float average) {
