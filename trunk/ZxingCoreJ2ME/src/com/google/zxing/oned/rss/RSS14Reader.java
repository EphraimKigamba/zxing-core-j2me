...
//    @Override
    public Result decodeRow(int rowNumber,
        ...
        for (int i = 0; i < lefSize; i++) {
            Pair left = (Pair) possibleLeftPairs.elementAt(i);
            if (left.getCount() > 1) {
                int rightSize = possibleRightPairs.size();
                for (int j = 0; j < rightSize; j++) {
                    Pair right = (Pair) possibleRightPairs.elementAt(j);
                    if (right.getCount() > 1) {
    ...
    private static void addOrTally(Vector possiblePairs, Pair pair) {
        ...
        for (int i = 0; i < possiblePairs.size(); i++) {
            Pair other = (Pair) possiblePairs.elementAt(i);
...
//  @Override
    public void reset() {
        possibleLeftPairs.removeAllElements();
        possibleRightPairs.removeAllElements();
    }
