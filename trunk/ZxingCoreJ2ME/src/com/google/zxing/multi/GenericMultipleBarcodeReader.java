...
/**
 * <p>Attempts to locate multiple barcodes in an image by repeatedly decoding
 * portion of the image. After one barcode is found, the areas left, above,
 * right and below the barcode's {@link ResultPoint}s are scanned,
 * recursively.</p>
 *
 * <p>A caller may want to also employ {@link ByQuadrantReader} when attempting
 * to find multiple 2D barcodes, like QR Codes, in an image, where the presence
 * of multiple barcodes might prevent detecting any one of them.</p>
 *
...
//  @Override
    public Vector decodeMultiple(BinaryBitmap image, Hashtable hints)
    ...
    private void doDecodeMultiple(BinaryBitmap image,
        ...
        for (int i = 0; i < resultPoints.length; i++) {
            ...
            float x = resultPoints[i].getX();
            float y = resultPoints[i].getY();
            if (x < minX) {
                minX = x;
            }
            if (y < minY) {
