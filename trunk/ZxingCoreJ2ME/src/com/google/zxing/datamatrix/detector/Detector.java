...
/**
 * <p>Encapsulates logic that can detect a Data Matrix Code in an image, even if
 * the Data Matrix Code is rotated or skewed, or partially obscured.</p>
 *
 * @author Sean Owen
 */
...
//////////        Collections.sort(transitions, new ResultPointsAndTransitionsComparator());
        ...
        Enumeration en=pointCount.keys();
        while(en.hasMoreElements()){
            ResultPoint point=(ResultPoint)en.nextElement();
...
////        }
        ...
        corners.addElement(maybeTopLeft);
        corners.addElement(bottomLeft);
        corners.addElement(maybeBottomRight);
        
        // Use the dot product trick to sort them out
        ResultPoint.orderBestPatterns(corners);

        // Now we know which is which:
        ResultPoint bottomRight = (ResultPoint)corners.elementAt(0);
        bottomLeft = (ResultPoint)corners.elementAt(1);
        ResultPoint topLeft = (ResultPoint)corners.elementAt(2);
    ...
    private static void increment(Hashtable table, ResultPoint key) {
        ...
        table.put(key, value == null ? new Integer(1) : new Integer(value.intValue() + 1));
    }
