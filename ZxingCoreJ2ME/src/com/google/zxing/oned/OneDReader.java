    ...
    public Result decode(BinaryBitmap image,
        ...
        try {
            ...
            if (tryHarder && image.isRotateSupported()) {
                ...
                int orientation = 270;
                if (metadata != null && metadata.containsKey(new Integer(ResultMetadataType.ORIENTATION))) {
                    // But if we found it reversed in doDecode(), add in that result here:
                    orientation = (orientation
                            + ((Integer) metadata.get(new Integer(ResultMetadataType.ORIENTATION))).intValue()) % 360;
                }
...
//              newHints.putAll(hints);
    ...
    protected static void recordPattern(BitArray row,
        ...
        Library.fill(counters, 0, numCounters, 0);
        int end = row.getSize();
        if (start >= end) {
            throw NotFoundException.getNotFoundInstance();
        }
     ...
     * total pattern size, where the ratio has been multiplied by 256. So, 0
     * means no variance (perfect match); 256 means the total variance between
     * counters and patterns equals the pattern length, higher values mean even
     * more variance
     */
    ...
    /**
     * <p>Attempts to decode a one-dimensional barcode format given a single row
     * of an image.</p>
     *
