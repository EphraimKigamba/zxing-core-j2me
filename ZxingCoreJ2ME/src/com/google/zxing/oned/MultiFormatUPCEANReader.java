...
//////        (Collection<BarcodeFormat>) hints.get(DecodeHintType.POSSIBLE_FORMATS);
    ...
    this.readers=new UPCEANReader[readers.size()];
    for(int i=0;i<readers.size();i++){
        this.readers[i]=(UPCEANReader)readers.elementAt(i);
    }
...
//  @Override
  public Result decodeRow(int rowNumber,
    ...
    for(int i=0;i<readers.length;i++){
      Result result;
      try {
        result = readers[i].decodeRow(rowNumber, row, startGuardPattern, hints);
...
////          hints == null ? null : (Collection<BarcodeFormat>) hints.get(DecodeHintType.POSSIBLE_FORMATS);
      Vector possibleFormats =
          hints == null ? null : (Vector) hints.get(DecodeHintType.POSSIBLE_FORMATS);
