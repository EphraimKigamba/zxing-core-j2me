...
//  @Override
    public BitMatrix encode(String contents,
        ...
        Writer writer = null;
        if (format.equals(BarcodeFormat.EAN_8)) {
            writer = new EAN8Writer();
        } else if (format.equals(BarcodeFormat.EAN_13)) {
            writer = new EAN13Writer();
        } else if (format.equals(BarcodeFormat.UPC_A)) {
            writer = new UPCAWriter();
        } else if (format.equals(BarcodeFormat.QR_CODE)) {
            writer = new QRCodeWriter();
        } else if (format.equals(BarcodeFormat.CODE_39)) {
